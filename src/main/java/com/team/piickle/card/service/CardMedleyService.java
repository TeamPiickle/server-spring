...
public class CardMedleyService {
    ...
    public List<CardMedleyPreviewResponseDto> getPreviewById() {
        ...
        return allMedley.stream()
                .map(
                        cardMedley -> {
                            List<CardMedleyPreviewResponseDto.PreviewCard> previewCards =
                                    cardRepository.findAllByIdIn(cardMedley.getPreviewCards()).stream()
    ...
    public CardMedleyResponseDto getCardsById(String id, Optional<String> userEmail) {
        CardMedley cardMedley =
                cardMedleyRepository
                        .findById(id)
                        .orElseThrow(() -> new GeneralException("해당하는 아이디의 카드 메들리가 없습니다."));
        List<CardMedleyPreviewResponseDto.PreviewCard> previewCardList =
                cardMedley.getPreviewCards().stream()
                        .map(
                                value -> {
                                    Card card =
                                            cardRepository
                                ...
                                card -> {
                                    ...
                                    if (!userEmail.isEmpty()) {
                                        ...
                                        if (bookmarkRepository
                                                .findByUserAndCard(
                                                        new ObjectId(user.getId()), new ObjectId(findCard.getId()))
                                                .isEmpty()) {
                                            bookmarked = false;
                                        }
                                    }
        ...
        return new CardMedleyResponseDto(
                CardMedleyPreviewResponseDto.of(cardMedley, previewCardList), cardResponseDtos);
    }
}
